...
//        List<Student> studetnList =  Stream.generate(() -> Student.getRandomStudent(pymc, jmc))
//                .limit(5000)
//                .toList();

//        List<Student> students = Stream
//                .iterate(1, s -> s <= 500, s -> s+1)
//                .map(s -> Student.getRandomStudent(pymc, jmc))
//                .toList();
...
//                .collect(() -> new TreeSet<Student>(uniqueSorted), TreeSet::add, TreeSet::addAll)
                .forEach(s -> {
                        s.addCourse(jgames);
                        System.out.print(s.getStudentId() + " ");
                    });
